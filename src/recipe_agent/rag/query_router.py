...
from recipe_agent.common import QueryAnalysisCache, LLMService, parallel_execute
...
class IntelligentQueryRouter:
    ...
    def analyze_query(self, query: str) -> QueryAnalysis:
        ...
        try:
            result = llm_service.chat_with_json([{"role": "user", "content": analysis_prompt}])
            ...
            self._query_cache.set(query, analysis)
    ...
    def _combined_search(self, query: str, top_k: int) -> List[Document]:
        """
        组合搜索策略：结合传统检索和图RAG的优势（并行化执行）
        """
        traditional_k = max(1, top_k // 2)
        graph_k = top_k - traditional_k

        def traditional_search():
            return self.traditional_retrieval.hybrid_search(query, traditional_k)

        def graph_search():
            return self.graph_rag_retrieval.graph_rag_search(query, graph_k)

        results = parallel_execute([traditional_search, graph_search], max_workers=2)

        traditional_docs = results[0] if results and results[0] else []
        graph_docs = results[1] if results and results[1] else []
