...
from recipe_agent.common import Neo4jConnectionPool, LLMService, QueryAnalysisCache, parallel_execute
...
class GraphRAGRetrieval:
    ...
    def __init__(self, config, llm_client):
        ...
        self._query_cache = QueryAnalysisCache()
    ...
    def initialize(self):
        ...
        try:
            from recipe_agent.common import Neo4jConnectionConfig
            ...
            self._neo4j_pool = Neo4jConnectionPool().initialize(neo4j_config)
            self._driver = self._neo4j_pool.driver

            if self._neo4j_pool.is_healthy:
                logger.info("Neo4j连接成功")
            else:
                logger.warning("Neo4j连接不可用，将使用降级方案")
    ...
    def understand_graph_query(self, query: str) -> GraphQuery:
        ...
        try:
            llm_service = LLMService(model_name=self.config.llm_model, temperature=0.1, max_tokens=1000)
            result = llm_service.chat_with_json([{"role": "user", "content": prompt}])
            ...
            self._query_cache.set(query, graph_query)
            return graph_query
    ...
    def multi_hop_traversal(self, graph_query: GraphQuery) -> List[GraphPath]:
        ...
        if not self._neo4j_pool or not self._neo4j_pool.is_healthy:
        ...
        try:
            ...
            if graph_query.query_type == QueryType.MULTI_HOP:
                ...
                results = self._neo4j_pool.execute_query(cypher_query, params)
                ...
                paths.extend(self._find_entity_relations(graph_query))

            elif graph_query.query_type == QueryType.PATH_FINDING:
                paths.extend(self._find_shortest_paths(graph_query))
